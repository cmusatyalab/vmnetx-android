...
Java_org_olivearchive_vmnetx_android_SpiceCommunicator_SpiceClientDisconnect (JNIEnv * env, jobject  obj, jlong context) {
    ...
    if (g_main_loop_is_running(ctx->thr->mainloop))
...
Java_org_olivearchive_vmnetx_android_SpiceCommunicator_SpiceClientConnect (JNIEnv *env, jobject obj, jlong context, jstring pw)
{
    ...
    ctx->thr->jni_get_fd           = (*env)->GetMethodID(env, cls, "OnGetFd", "(J)V");
    ctx->thr->jni_settings_changed = (*env)->GetMethodID(env, cls, "OnSettingsChanged", "(IIII)V");
    ctx->thr->jni_graphics_update  = (*env)->GetMethodID(env, cls, "OnGraphicsUpdate", "(IIIII)V");
    ctx->thr->jni_cursor_config    = (*env)->GetMethodID(env, cls, "OnCursorConfig", "(Z)V");
    ...
    g_type_init();
    ctx->thr->mainloop = g_main_loop_new(NULL, false);
    ...
    if (ctx->connections > 0) {
        g_main_loop_run(ctx->thr->mainloop);
        connection_disconnect(conn);
        g_object_unref(ctx->thr->mainloop);
    ...
    ctx->thr->jni_get_fd           = NULL;
    ctx->thr->jni_settings_changed = NULL;
    ctx->thr->jni_graphics_update  = NULL;
    ctx->thr->jni_cursor_config    = NULL;
    ctx->thr->jenv                 = NULL;
}
...
Java_org_olivearchive_vmnetx_android_SpiceCommunicator_SpiceClientNewContext (JNIEnv *env, jobject obj)
{
    ...
    ctx->thr = g_slice_new0(struct spice_main_thread);
    return (jlong) ctx;
}
...
{
    struct spice_context *ctx = (struct spice_context *) context;
    g_slice_free(struct spice_main_thread, ctx->thr);
